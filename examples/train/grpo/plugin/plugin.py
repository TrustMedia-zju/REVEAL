...
import deepspeed
from fuliye import compute_fuliye_spectrum, get_fuliye_image_rgb
def resize_image(image_path, max_size=(1024, 1024)):
    """
    读取图像并根据最大尺寸进行调整，如果图像超过最大尺寸。
    ...
    if width > max_size[0] or height > max_size[1]:
        img.thumbnail(max_size)  # 保持比例缩放图像
...
# # 3. 加载模型（注意：不要用 device_map="auto"）
# modeltest = AutoModelForCausalLM.from_pretrained(
#     model_name,
#     torch_dtype=torch.bfloat16 if torch.cuda.is_bf16_supported() else torch.float16
# )

# # 4. 初始化 DeepSpeed 推理引擎
# ds_engine = deepspeed.init_inference(
#     model=modeltest,
#     mp_size=8,                     # 你的 GPU 数量（比如 8）
#     dtype=torch.bfloat16,          # 推荐使用 bf16
#     replace_method='auto',         # 自动替换推理 kernel
#     replace_with_kernel_inject=True
# )

# modeltest = ds_engine.module  # DeepSpeed 封装后的模型对象
# def compare_image_analysis_en(text1: str, text2: str, modeltest, max_new_tokens=4096):

#     """
#     Use Qwen model to compare two image analysis texts.
    
#     Args:
#         text1 (str): First analysis text
#         text2 (str): Second analysis text
#         model_name (str): Model name
#         max_new_tokens (int): Max tokens to generate
    
#     Returns:
#         int: 1 (consistent), 0 (partially consistent), -1 (opposite)
#     """
...
#     # English prompt
#     prompt = (
#         "You will be given two paragraphs analyzing an image for authenticity. "
#         "Compare their analysis and conclusion. "
#         "If the analyses and conclusions are consistent, output 1. "
#         "If they are partially consistent, output 0. "
#         "If they are opposite, output -1. "
#         "Do not output anything else. You can reason internally but only return the final number."
#     )
    
#     messages = [
#         {"role": "user", "content": prompt},
#         {"role": "user", "content": f"Text1: {text1}"},
#         {"role": "user", "content": f"Text2: {text2}"}
#     ]
    
#     # Prepare input for model
#     text = tokenizertest.apply_chat_template(
#         messages,
#         tokenize=False,
#         add_generation_prompt=True,
#         enable_thinking=True  # internal reasoning allowed
#     )
#     model_inputs = tokenizertest([text], return_tensors="pt").to(modeltest.device)
#     # Generate output
#     generated_ids = modeltest.generate(
#         **model_inputs,
#         max_new_tokens=max_new_tokens
#     )

#     # Extract output tokens after input
#     output_ids = generated_ids[0][len(model_inputs.input_ids[0]):].tolist()
    
#     # Find </think> token id (internal reasoning)
#     try:
#         index = len(output_ids) - output_ids[::-1].index(151668)  # </think>
#     except ValueError:
#         index = 0
    
#     # thinking content (internal, can ignore)
#     _thinking_content = tokenizertest.decode(output_ids[:index], skip_special_tokens=True).strip("\n")
    
#     # final output
#     content = tokenizertest.decode(output_ids[index:], skip_special_tokens=True).strip()
    
#     # Parse to int
#     try:
#         result = int(content)
#         if result not in (-1, 0, 1):
#             result = 0  # fallback to 0 if unexpected
#     except ValueError:
#         result = 0
...
#     torch_dtype=torch.bfloat16 if torch.cuda.is_bf16_supported() else torch.float16
# )
modeltest =  Qwen2_5_VLForConditionalGeneration.from_pretrained(
    model_name,
    torch_dtype=torch.bfloat16 if torch.cuda.is_bf16_supported() else torch.float16
)
processortest = AutoProcessor.from_pretrained(model_name)
...
modeltest = ds_engine.module  # DeepSpeed 封装后的模型对象
def shuffle_reasoning_steps(split_text):
    """
    随机打乱前 n-1 段推理内容，保持最后一段（通常是结论）不动。
    适用于任何长度的列表。
    """
    if len(split_text) <= 1:
        return split_text  # 只有一个段落时不打乱
    
    reasoning_steps = split_text[:-1]   # 除最后一个以外
    conclusion = split_text[-1:]        # 最后一个（结论）
    
    shuffled_reasoning = random.sample(reasoning_steps, len(reasoning_steps))
    shuffled_text = shuffled_reasoning + conclusion
    return shuffled_text
def split_points(text):
    """
    将文本按数字开头分点拆分
    e.g. "1. First 2. Second 3. Third" -> ["First", "Second", "Third"]
    """
    # 1. 匹配数字和句号开头的部分，直到下一个数字加句号或文本结束
    pattern = r'(\d+)\.\s*(.*?)(?=\s*\d+\.\s*|\Z)'  # 匹配数字后面跟着句号和空格，非贪婪地获取内容
    matches = re.findall(pattern, text, re.DOTALL)  # DOTALL模式使'.'匹配换行符
    
    # 2. 返回拆分后的段落，只保留文本部分
    return [m[1].strip() for m in matches]
def compare_image_analysis_flexible(text1: str, text2: str, modeltest, max_new_tokens=1024):
    """
    分点对比文本，如果点数不一样则退回整体文本对比
    """
    # print(text1)
    # print(text2)
    # print(1/0)
    points1 = split_points(text1)
    points2 = split_points(text2)
    ...
    if len(points1) != len(points2):
        return_text=compare_image_analysis_en(text1, text2, modeltest, max_new_tokens=max_new_tokens)
        if return_text == 1:
            return 1.0
        elif return_text == 0:
            return 0.5
    ...
    for p1, p2 in zip(points1, points2):
        try:
            prompt = (
                "Compare the following two analysis points for consistency. "
                "If they are consistent, return 1; if partially consistent, return 0; if opposite, return -1. "
                "Only return the number."
            )
            messages = [
                {"role": "user", "content": prompt},
                {"role": "user", "content": f"Point1: {p1}"},
                {"role": "user", "content": f"Point2: {p2}"}
            ]

            # 准备模型输入
            text_input = tokenizertest.apply_chat_template(
                messages, tokenize=False, add_generation_prompt=True, enable_thinking=True
            )
            model_inputs = tokenizertest([text_input], return_tensors="pt").to(modeltest.device)
            generated_ids = modeltest.generate(**model_inputs, max_new_tokens=max_new_tokens)
            output_ids = generated_ids[0][len(model_inputs.input_ids[0]):].tolist()
            ...
            try:
                ...
                if score==-1:
                    ...
                    sub_reward=1.0
                # score = 1 if score == 1 else 0  # 只给完全一致奖励
            except:
                score = 0
    ...
    return sum(rewards) / len(rewards)+0.2
def compare_image_analysis_en(text1: str, text2: str, modeltest, max_new_tokens=4096):
    ...
    return result
def compare_image_text_en(text1: str, image: str, modeltest, max_new_tokens=4096):
    messages = []
    max_size=(1024, 1024)
    image_path=image
    image_1024 = resize_image(image_path, max_size=max_size)
    fuliye = get_fuliye_image_rgb(compute_fuliye_spectrum(image))
    
    # 转换为 numpy 数组并从 RGB 转换为 BGR（如果需要）
    fuliye_np = np.array(fuliye)
    fuliye_bgr = cv2.cvtColor(fuliye_np, cv2.COLOR_RGB2BGR)
    
    # 你可能需要将这个 BGR 图像转换回 PIL 图像或其他格式，取决于模型的需求
    fuliye_pil = Image.fromarray(cv2.cvtColor(fuliye_bgr, cv2.COLOR_BGR2RGB))  # 转换为 RGB 格式
    
    # 获取图像尺寸
    width, height = fuliye_pil.size
    
    # 如果图像尺寸超过最大尺寸，则进行缩放
    if width > max_size[0] or height > max_size[1]:
        fuliye_pil.thumbnail(max_size)  # 保持比例缩放图像
    messages.append({
        "role": "user",
        "content": [{"type": "image", "image": fuliye_pil}],  # 传入处理后的图像
    })
    srm_result = SRM(image)  # 假设 SRM 函数返回处理后的图像
    
    # # 将 SRM 处理后的图像转换为 PIL 格式
    srm_pil = Image.fromarray(srm_result)  # 假设 SRM 返回的是 NumPy 数组
    width, height = srm_pil.size
    
    # 如果图像尺寸超过最大尺寸，则进行缩放
    if width > max_size[0] or height > max_size[1]:
        srm_pil.thumbnail(max_size)  # 保持比例缩放图像
    messages.append({
        "role": "user",
        "content": [{"type": "image", "image": srm_pil}],  # 传入SRM图像
    })

    prompt = (
    "You will be given a paragraph that analyzes whether an image is real or AI-generated. "
    "You are also provided with several visual variants of the same image, including: "
    "the original image, a high-pass filtered version, a frequency spectrum map, and a magnified local region. "
    "Your task is to evaluate whether the textual analysis accurately and credibly reflects the actual visual evidence across these images. "
    "If the description fully aligns with the observed visual traces (e.g., texture inconsistencies, frequency artifacts, lighting anomalies), output 1. "
    "If the description is partially consistent with the images, output 0. "
    "If the description contradicts or does not match the image evidence at all, output -1. "
    "Do not output anything else. You may reason internally, but only return the final number."
    )
    ...
    "content": [
        ...
        {"type": "text", "text": prompt},  # 传入任务指令
        {"type": "text", "text": text1},   # 传入分析文本
    ],
    })

    text = processortest.apply_chat_template(
        messages, tokenize=False, add_generation_prompt=True
    )
    image_inputs, video_inputs = process_vision_info(messages)
    inputs = processortest(
        text=[text],
        images=image_inputs,
        videos=video_inputs,
        padding=True,
    ...
    generated_ids = modeltest.generate(**inputs, max_new_tokens=1024)
    generated_ids_trimmed = [
        out_ids[len(in_ids) :] for in_ids, out_ids in zip(inputs.input_ids, generated_ids)
    ]
    output_text = processortest.batch_decode(
        generated_ids_trimmed, skip_special_tokens=True, clean_up_tokenization_spaces=False
    )

    try:
        result = int(output_text[0].strip())  # 直接将解码的文本转换为整数
        if result not in (-1, 0, 1):
            result = 0  # 如果值不在 -1, 0, 1 之间，则默认为 0
    except ValueError:
        result = 0  # 如果转换失败，设置为默认值 0

    return result
def compare_image_text_logic(text1: str, text2: str, modeltest, max_new_tokens=4096):
       # English prompt
    split_text=split_points(text1)
    shuffled_list = shuffle_reasoning_steps(split_text)
    shuffled_output = "\n".join(shuffled_list)



    prompt = (
        "You will be given two texts analyzing the authenticity of an image. "
        "The first text follows the original reasoning order, while the second text is a shuffled version of the reasoning process. "
        "Compare the two and evaluate how the reasoning order affects the validity of the conclusion. "
        "If the shuffled text still fully supports the same conclusion, output -1 (logic not strongly related). "
        "If the shuffled text slightly affects the reasoning but still generally supports the conclusion, output 0 (weak logic). "
        "If the shuffled text becomes inconsistent or fails to support the conclusion, output 1 (strong logic). "
        "Do not output anything else. You can reason internally but only return the final number."
    )
    ...
    messages = [
        {"role": "user", "content": prompt},
        {"role": "user", "content": f"Text1: {text1}"},
        {"role": "user", "content": f"Text2: {shuffled_output}"}
    ]
...
class MultiModalAccuracyORM(ORM):
    def __call__(self, completions, solution ,**kwargs) -> List[float]:
        ...
        # print(kwargs["images"][0][0]['path'])
        image = kwargs["images"][0][0]['path']
        # srm=SRM(kwargs["images"][0][0]['path'])
        # cv2.imwrite("image1.png", cv2.cvtColor(srm, cv2.COLOR_RGB2BGR))
        # print("已保存为 image1.png")
        # fuliye=get_fuliye_image_rgb(compute_fuliye_spectrum(kwargs["images"][0][0]['path']))
        # fuliye_np = np.array(fuliye)
        # fuliye_bgr = cv2.cvtColor(fuliye_np, cv2.COLOR_RGB2BGR)
        
        # # 保存为 PNG 文件
        # cv2.imwrite("image2.png", fuliye_bgr)
        # print("已保存为 image2.png")
        # # cv2.imwrite("image2.png", cv2.cvtColor(fuliye, cv2.COLOR_RGB2BGR))
        # print(fuliye)
        for content, sol in zip(completions, solution):
            # print(content)
            # print(1/0)
            answer_reward = 0.0
            think_format_reward = 0.0
            think_semantic_reward = 0.0
            think_is_truth_image=0.0
            think_is_logic=0.0
            think_step=0.0
            # -------------------------------
            # 1. 提取 <think> 标签
            # -------------------------------
            think_match_content = re.search(r'<think>(.*?)</think>', content, re.DOTALL)
            think_match_sol = re.search(r'<think>(.*?)</think>', sol, re.DOTALL)

            think_content = think_match_content.group(1).strip() if think_match_content else ""
            think_sol = think_match_sol.group(1).strip() if think_match_sol else ""
  
            # -------------------------------
            # 2. 验证 <think> 是否分点作答
            #    简单规则：每行开头是否包含数字或符号，如 1. 2. * -
            # -------------------------------
            points_count = len(re.findall(r'\b\d+\b', think_content))
            if points_count >= 3:
            ...
            # -------------------------------            
            try:
                return_text = compare_image_analysis_en(think_content, think_sol, modeltest)
                
                if return_text == 1:
                    think_semantic_reward = 1.0
                elif return_text == 0:
                    think_semantic_reward = 0.5
                elif return_text == -1:
                    think_semantic_reward = 0
                else:
                    think_semantic_reward = 0  # 默认奖励为0

            except Exception as e:
                print(f"Error occurred: {e}")
                think_semantic_reward = 0  # 出现错误时奖励保持0
            ...
            try:
                return_text = compare_image_text_en(think_content, image, modeltest)
            
                if return_text == 1:
                    think_is_truth_image = 1.0
                elif return_text == 0:
                    think_is_truth_image = 0.5
                elif return_text == -1:
                    think_is_truth_image = 0
                else:
                    think_is_truth_image = 0  # 默认奖励为0

            except Exception as e:
                print(f"Error occurred: {e}")
                think_is_truth_image = 0  # 出现错误时奖励保持0
            ...
            try:
                return_text = compare_image_text_logic(think_content, think_sol, modeltest)
                ...
                if return_text == 1:
                    ...
                    think_is_logic = 0  # 默认奖励为0

            except Exception as e:
                print(f"Error occurred: {e}")
                think_is_logic = 0  # 出现错误时奖励保持0
            # print(think_is_logic)
            # -------------------------------
            # 7. 分布奖励
            # -------------------------------
            # think_step=compare_image_analysis_flexible(think_content, think_sol, modeltest)
            # -------------------------------
            # 8. 总奖励
            # -------------------------------
            # print(think_is_truth_image)
            total_reward = answer_reward +0.1*think_format_reward+0.3*think_semantic_reward+0.2*think_is_truth_image+0.2*think_is_logic
    ...
    #         # -------------------------------            
    #         try:
    #             think_semantic_reward = compare_image_analysis_flexible(think_content, think_sol, modeltest)
    ...
    #         # 5. 总奖励
    #         # -------------------------------
    #         total_reward = answer_reward + 0.4*think_format_reward + 0.6*think_semantic_reward
    #         rewards.append(total_reward)
